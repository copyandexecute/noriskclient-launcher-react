...
const getDataPackFileName = (pack: DataPackInfo | null | undefined): string => {
  if (!pack) return "Unknown Data Pack";
  ...
  return pack.path || "Unknown Data Pack";
};
...
export function DataPacksTabV2({ profile, onRefreshRequired }: DataPacksTabV2Props) {
  ...
  } = useLocalContentManager<DataPackInfo>({
    profile,
    contentType: 'DataPack',
    getDisplayFileName: getDataPackFileName,
    onRefreshRequired,
  });
  ...
  const renderDataPackItem = useCallback((pack: DataPackInfo) => {
    const itemTitle = getDataPackFileName(pack);
    ...
    if (modrinthIconUrl) {
      ...
      iconToShow = <img src={localIconDataUrl} alt={`${itemTitle} icon`} className="w-full h-full object-contain image-pixelated"/>;
    } else {
    ...
    const itemBadgesNode = (
      <>
        <TagBadge 
            ...
            iconElement={!pack.is_disabled ? <Icon icon={DATA_PACKS_TAB_ICONS_TO_PRELOAD[3]} className="w-3 h-3"/> : <Icon icon={DATA_PACKS_TAB_ICONS_TO_PRELOAD[4]} className="w-3 h-3"/>}
        >
    ...
    if (updateAvailableVersion && !isCurrentlyUpdating) {
      ...
      itemUpdateActionNode = <IconButton 
        size="sm" 
    ...
    const itemDropdownNode = (
      <div 
        ...
        <button 
          ...
          <Icon icon="solar:folder-open-bold-duotone" className="w-3 h-3 flex-shrink-0" /> Open Folder
        </button>
      </div>
    );
    ...
    <div className="flex flex-col gap-2 flex-grow min-w-0">
      <div className="flex items-center gap-2">
        ...
        <IconButton 
            ...
            colorScheme="secondary"
            size="sm" 
            title={isLoading ? "Refreshing..." : "Refresh Data Packs"} 
  ...
  if (!profile) {
    return (
      <div className="p-4 font-minecraft text-center text-white/70">
        Profile data became unavailable. Cannot display data packs.
      </div>
    );
  }
  ...
  return (
    <>
      <GenericContentTab<DataPackInfo> 
        items={filteredItems}
        renderListItem={renderDataPackItem} 
        isLoading={isLoading}
        error={error}
        ...
        emptyStateMessage={
          ...
          !searchQuery && dataPacks.length === 0 && selectedItemIds.size === 0 ? "No data packs found." :
          searchQuery && filteredItems.length === 0 && selectedItemIds.size === 0 ? "No data packs match your search." :
          "Manage your data packs"
        }
        emptyStateDescription={
          error ? "Please try refreshing." :
          isLoading && dataPacks.length === 0 ? "Please wait..." :
          !searchQuery && dataPacks.length === 0 && selectedItemIds.size === 0 ? "Add data packs to this profile via its 'datapacks' folder." :
          searchQuery && filteredItems.length === 0 && selectedItemIds.size === 0 ? "Try a different search." :
          "Select packs for batch actions or manage them individually."
        }
      ...
      <ConfirmDeleteDialog 
        ...
        itemName={itemToDeleteForDialog ? getDataPackFileName(itemToDeleteForDialog) : `${selectedItemIds.size} data pack${selectedItemIds.size === 1 ? '' : 's'}`}
        onClose={handleCloseDeleteDialog}
        onConfirm={handleConfirmDeletion}
        isDeleting={isDialogActionLoading}
        title={itemToDeleteForDialog ? `Delete ${getDataPackFileName(itemToDeleteForDialog)}?` : "Delete Selected Data Packs?"}
      />
    </>
  );
} 