...
import type { NoriskModpacksConfig, NoriskModEntryDefinition, NoriskModSourceDefinition } from "../../../../types/noriskPacks"; // Changed NoriskPackMod to NoriskModEntryDefinition
...
export function NoRiskModsTabV2({ profile, onRefreshRequired }: NoRiskModsTabV2Props) {
  if (!profile) {
  ...
  const lastLoadedPackIdRef = useRef<string | null | undefined>(profile.selected_norisk_pack_id);

  // Moved data fetching and processing logic before useEffects that depend on them
  const isNoriskModDisabled = useCallback((packModId: string): boolean => {
  ...
  }, [profile.selected_norisk_pack_id, profile.disabled_norisk_mods_detailed, profile.game_version, profile.loader]);

  const fetchModIconsForNoRisk = useCallback(async (compatibleRawMods: NoriskModEntryDefinition[]) => {
  ...
  const processFetchedMods = useCallback(async (rawMods: NoriskModEntryDefinition[], currentPacksConfig: NoriskModpacksConfig) => {
    ...
    const processedMods: NoRiskModV2[] = compatibleRawMods.map(rawMod => {
      ...
      if (rawMod.compatibility && rawMod.compatibility[profile.game_version] && rawMod.compatibility[profile.game_version][profile.loader]) {
        const target = rawMod.compatibility[profile.game_version][profile.loader];
        if (target) {
  ...
  }, [isNoriskModDisabled, profile.game_version, profile.loader, fetchModIconsForNoRisk]);
    ...
    try {
      ...
      if (!currentPacksConfig) {
        try {
          ...
          console.warn("Failed to get resolved NoRisk packs, trying basic:", resolvedError);
      ...
      if (profile.selected_norisk_pack_id && currentPacksConfig) {
        ...
        try {
          const modsResult = await invoke<NoriskModEntryDefinition[]>("get_norisk_pack_mods", {
          ...
          if (Array.isArray(modsResult)) {
            ...
            console.warn("Unexpected response from get_norisk_pack_mods, not an array:", modsResult);
            const packDef = currentPacksConfig.packs[profile.selected_norisk_pack_id];
            if (packDef?.mods) {
              rawModsData = packDef.mods;
            }
          }
        } catch (directError) {
          console.warn("get_norisk_pack_mods failed, trying fallback to pack definition:", directError);
          const packDef = currentPacksConfig.packs[profile.selected_norisk_pack_id];
          if (packDef?.mods) {
            rawModsData = packDef.mods;
          } else {
            console.warn("Pack definition has no mods, trying list_norisk_mods for profile:", profile.id);
            try {
                const lastResortResult = await invoke<NoriskModEntryDefinition[]>("list_norisk_mods", {
                    profileId: profile.id,
                });
                if(Array.isArray(lastResortResult)) {
                    rawModsData = lastResortResult;
                } else {
                    console.error("list_norisk_mods also returned unexpected data:", lastResortResult);
                }
            } catch (lastResortError) {
                console.error("list_norisk_mods also failed:", lastResortError);
                throw new Error(`Failed to load NoRisk mods. Pack: ${profile.selected_norisk_pack_id}. Error: ${lastResortError}`);
            }
          }
        }
        ...
        if (rawModsData.length > 0) {
            ...
            setNoriskMods([]);
            setLocalIcons({});
            console.log("No NoRisk mods found for pack:", profile.selected_norisk_pack_id);
        }
      ...
      setError(`Failed to load NoRisk data: ${err instanceof Error ? err.message : String(err)}`);
      setNoriskMods([]);
      setLocalIcons({});
    } finally {
      setIsLoading(false);
    }
  }, [profile.id, profile.selected_norisk_pack_id, profile.game_version, profile.loader, noriskPacksConfig, processFetchedMods]);
  ...
  useEffect(() => {
    const setupEventListeners = async () => {
      const unlisten = await listen<any>("state_event", (event) => {
        ...
        if (payload.event_type === "trigger_profile_update" && payload.target_id === profile.id) {
  ...
  const handleRefreshPacks = async () => {
    ...
    try {
      ...
      setError(`Failed to refresh NoRisk packs: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
      setIsRefreshingPacks(false);
    }
  };
  
  const handleToggleNoRiskMod = useCallback(async (modId: string) => {
    ...
    setNoriskMods(prevMods => 
      prevMods.map(m => m.id === modId ? { ...m, enabled: newEnabledState } : m)
    );
    ...
    try {
      ...
      toast.error(`Failed to toggle ${mod.display_name}: ${err instanceof Error ? err.message : String(err.message)}`);
      setNoriskMods(prevMods => 
        prevMods.map(m => m.id === modId ? { ...m, enabled: mod.enabled } : m) 
      );
    } finally {
      setModBeingToggled(null);
    }
  }, [noriskMods, profile, onRefreshRequired]); // Keep onRefreshRequired in deps if other parts of the callback chain might still need it, though we are not calling it.
  ...
  const filteredMods = useMemo(() => {
    ...
    if (searchQuery) {
      modsToFilter = noriskMods.filter((mod) => 
        (mod.display_name || mod.id).toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const handleBatchToggleSelected = async () => {
    ...
    const modsToToggle = Array.from(selectedModIds).map(id => noriskMods.find(m => m.id === id)).filter(Boolean) as NoRiskModV2[];
    ...
    for (const mod of modsToToggle) {
      ...
      try {
        ...
        successfulToggles++;
        setNoriskMods(prev => prev.map(m => m.id === mod.id ? {...m, enabled: newEnabledState} : m));
      } catch (err) {
    ...
    if (errors.length > 0) {
      console.warn("Batch NoRisk mod toggle finished with errors:", errors.join("; "));
    }
  };

  const handleSelectedPackChange = async (newPackId: string | null) => { // Allow null for unsetting
  ...
  const noriskPackOptions = useMemo((): SelectOption[] => {
    if (!noriskPacksConfig) return [{ value: "", label: "- No Pack Selected -" }]; // Return default if no config
  ...
  const renderNoRiskModItem = useCallback((mod: NoRiskModV2) => {
    ...
    if (localIconData) {
      ...
      iconToShow = <Icon icon={NORISK_MODS_TAB_ICONS_TO_PRELOAD[0]} className="w-8 h-8 sm:w-10 sm:h-10 text-white/40" />;
    }
    const itemIconNode = (
    ...
    const sourceTypeDisplay: Record<Extract<NoriskModSourceDefinition['type'], 'modrinth' | 'maven' | 'url'>, {label: string, variant: "info" | "default" | "warning" }> = {
    ...
    const itemBadgesNode = (
      <>
        <TagBadge 
          ...
          iconElement={mod.enabled ? <Icon icon={NORISK_MODS_TAB_ICONS_TO_PRELOAD[3]} /> : <Icon icon={NORISK_MODS_TAB_ICONS_TO_PRELOAD[4]} />}
        >
          {mod.enabled ? "Enabled" : "Disabled"}
        </TagBadge>
        {mod.source_type && sourceTypeDisplay[mod.source_type as Extract<NoriskModSourceDefinition['type'], 'modrinth' | 'maven' | 'url'>] && (
          <TagBadge 
            size="sm" 
            variant={sourceTypeDisplay[mod.source_type as Extract<NoriskModSourceDefinition['type'], 'modrinth' | 'maven' | 'url'>].variant}
            className="ml-1 capitalize"
          >
            {sourceTypeDisplay[mod.source_type as Extract<NoriskModSourceDefinition['type'], 'modrinth' | 'maven' | 'url'>].label}
          </TagBadge>
        )}
      </>
    );
  ...
  const primaryLeftActionsContent = (
    <div 
      className="flex flex-col gap-2 flex-grow min-w-0"
    >
      ...
      <div className="flex items-center justify-between w-full min-h-14">
        ...
        <div className="flex items-center gap-2">
          {selectedModIds.size > 0 && !!profile.selected_norisk_pack_id && (
            ...
            >
              {isBatchToggling ? "Toggling..." : `Toggle Selected (${selectedModIds.size})`}
            </Button>
          )}
  ...
  return (
    <>
      <GenericContentTab<NoRiskModV2>
        ...
        emptyStateMessage={ 
          !profile.selected_norisk_pack_id ? "No NoRisk Pack Selected" :
          error ? "Error loading NoRisk mods" : 
          isLoading && filteredMods.length === 0 && !!profile.selected_norisk_pack_id ? "Loading NoRisk mods..." : 
          !searchQuery && filteredMods.length === 0 && !!profile.selected_norisk_pack_id ? "No mods in this NoRisk pack." : 
          searchQuery && filteredMods.length === 0 && !!profile.selected_norisk_pack_id ? "No NoRisk mods match your search." :
          "Manage your NoRisk mods"
        }
        emptyStateDescription={
          !profile.selected_norisk_pack_id ? "Please select a NoRisk Modpack from the dropdown above to manage its mods." : 
          error ? "Please try refreshing or check the console." :
          isLoading && filteredMods.length === 0 && !!profile.selected_norisk_pack_id ? "Please wait..." :
          !searchQuery && filteredMods.length === 0 && !!profile.selected_norisk_pack_id ? "This pack might be empty, or mods are still loading." : 
          searchQuery && filteredMods.length === 0 && !!profile.selected_norisk_pack_id ? "Try a different search term." :
          "Toggle mods on or off for this profile."
        }
        loadingItemCount={(isLoading && !!profile.selected_norisk_pack_id && noriskMods.length === 0) ? 5 : 0} 
        showSkeletons={false}
