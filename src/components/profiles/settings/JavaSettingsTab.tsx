...
export function JavaSettingsTab({
  ...
  const detectJavaInstallations = async () => {
    ...
    try {
      ...
      if (installations.length === 0) {
        toast("No Java installations found on your system. You may need to specify the path manually if you use a custom Java setup.");
  ...
  const testCustomJavaPath = async (path_to_test?: string) => {
    ...
    setValidationMessage(null);
    try {
      const isValid: boolean = await invoke("validate_java_path_command", { path: currentPath });
      if (isValid) {
        setValidationResult("valid");
        toast.success("Java path is valid!");
        updateProfile({ settings: { ...editedProfile.settings, java_path: currentPath, use_custom_java_path: true } });
      } else {
        setValidationResult("invalid");
        toast.error("Invalid Java Path. Check the path or ensure it's a compatible Java version.");
      }
    } catch (error: any) {
  ...
  const handleCustomJavaToggle = (checked: boolean) => {
    ...
    if (checked) {
      if (!customJavaPathInput && detectedJavaInstallations.length > 0) {
        const defaultOrFirst = detectedJavaInstallations.find(j => j.is_default) || detectedJavaInstallations[0];
        if (defaultOrFirst) setCustomJavaPathInput(defaultOrFirst.path);
      }
      updateProfile({ settings: { ...editedProfile.settings, use_custom_java_path: true } });
    } else {
      updateProfile({ settings: { ...editedProfile.settings, use_custom_java_path: false } });
      setValidationResult(null);
    }
  };
    ...
    <div ref={tabRef} className="space-y-6 select-none">
      <div ref={javaInstallRef} className="space-y-4">
        <div>
          ...
          {useCustomJava && (
            <Card variant="default" className="p-5 mt-3 space-y-4 custom-java-input">
              {isDetectingJava && (
              ...
              <div>
                ...
                <div className="flex gap-3">
                  ...
                  <Button
                    ...
                    shadowDepth="short"
                    icon={<Icon icon="solar:folder-with-files-bold" className="w-5 h-5 text-white" />}
                    className="text-2xl"
                  ...
                  <div className="max-h-40 overflow-y-auto custom-scrollbar space-y-1 pr-2">
                    {detectedJavaInstallations.map(java => (
                      <button
                        ...
                        title={java.path}
                        className={cn(
                          "w-full text-left p-2 border-2 transition-all duration-150 font-minecraft-ten text-xs rounded-md",
                          customJavaPathInput === java.path 
                            ? "bg-accent/30 border-accent text-white"
                            : "bg-black/20 border-white/20 hover:bg-black/30 hover:border-white/40 text-white/80"
                        )}
                        style={customJavaPathInput === java.path ? {
              ...
              <Button
                ...
                icon={ isValidatingJavaPath ? <Icon icon="solar:refresh-bold" className="w-5 h-5 animate-spin" /> : <Icon icon="solar:test-tube-bold" className="w-5 h-5"/> }
