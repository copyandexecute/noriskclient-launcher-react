...
const AccordionItem: React.FC<AccordionItemProps> = ({
  ...
  return (
    ...
    >
      ...
      {isOpen && <div className="p-2 text-sm relative z-10">{children}</div>}
    </div>
  );
};
...
export const ModrinthFilterSidebarV2: React.FC<
  ModrinthFilterSidebarV2Props
> = ({
  projectType,
  accentColor,
  ...
  return (
    <div
      className={cn(
        "filters-sidebar w-1/4 max-w-[15rem] flex-shrink-0 overflow-y-auto p-1 h-full -mt-1",
      ...
      <div className="space-y-3 pt-2">
          ...
          <div className="space-y-2">
            ...
            <div className="max-h-40 overflow-y-auto space-y-0.5 p-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isChecked
                        ? "bg-white/15 text-[color:var(--accent)]"
                        : "text-gray-300",
                    )}
              ...
              {displayedGameVersions.length === 0 && (
                <p className="text-gray-500 italic p-1 text-xs text-center">
            ...
            <div className="flex items-center mt-2">
              <Checkbox
                ...
                onChange={(e) =>
                  onShowAllGameVersionsSidebarChange(e.target.checked)
                }
              />
              <label
        ...
        {categoriesGroup && (
          ...
          >
            <div className="max-h-60 overflow-y-auto space-y-0.5 p-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              {categoriesGroup.options.length > 0 ? (
                categoriesGroup.options.map((cat) => {
                  const isChecked = currentSelectedCategories.includes(
                    cat.name,
                  );
                  return (
                    <button
                      ...
                      style={{
                        color: isChecked ? accentColor.value : undefined,
                      }}
                    >
                ...
                <p className="text-xs text-gray-500 italic p-1 text-center">
                  No options for {categoriesGroup.accordionTitle}.
                </p>
              )}
            </div>
          </AccordionItem>
        ...
        <AccordionItem title="Loader" defaultOpen={true}>
          <div className="max-h-40 overflow-y-auto space-y-0.5 p-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            {availableLoaders.map((loader) => {
              ...
              const fullLoaderData = allLoadersData.find(
                (l) => l.name === loader.name,
              );
              return (
                <button
                  key={loader.name}
                ...
                >
                  <span className="flex items-center flex-grow text-left">
                    {fullLoaderData?.icon &&
                      fullLoaderData.icon.trim() !== "" && (
                        <span
                          className="w-4 h-4 mr-1.5 flex-shrink-0"
                          dangerouslySetInnerHTML={{
                            __html: fullLoaderData.icon,
                          }}
                        />
                      )}
                    {loader.name}
        ...
        {otherDynamicGroups.map((group) => (
          <AccordionItem
            ...
            defaultOpen={
              group.headerValue.toLowerCase() ===
              "categories" /* This defaultOpen might need adjustment if categories is handled separately */
            }
          >
            <div className="max-h-60 overflow-y-auto space-y-0.5 p-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        ...
        <AccordionItem title="Environment" defaultOpen={false}>
          <div className="space-y-0.5 p-1">
            <button
              ...
              style={{
                color: filterClientRequired ? accentColor.value : undefined,
              }}
            >
              <span className="flex items-center flex-grow text-left">
                <Icon
            ...
            <button
              ...
              style={{
                color: filterServerRequired ? accentColor.value : undefined,
              }}
            >
              <span className="flex items-center flex-grow text-left">
                <Icon
