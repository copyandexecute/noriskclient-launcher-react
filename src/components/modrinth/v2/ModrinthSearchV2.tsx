...
const ALL_MODRINTH_PROJECT_TYPES: ModrinthProjectType[] = [
  "mod",
  "modpack",
  "resourcepack",
  "shader",
...
export function ModrinthSearchV2({
  ...
  const sortOptions: {
  ...
  }[] = [
    {
      value: "relevance",
      label: "Relevance",
      icon: <Icon icon="solar:sort-bold" className="w-4 h-4" />,
    },
    {
      value: "downloads",
      label: "Downloads",
    ...
    },
    {
      value: "follows",
      label: "Follows",
      icon: <Icon icon="solar:heart-bold" className="w-4 h-4" />,
    },
    {
      value: "newest",
      label: "Newest",
      icon: <Icon icon="solar:calendar-mark-bold" className="w-4 h-4" />,
    }, // Changed icon
    {
      value: "updated",
      label: "Updated",
  ...
  const [allCategoriesData, setAllCategoriesData] = useState<
    ModrinthCategory[]
  >([]);
  const [gameVersionsData, setGameVersionsData] = useState<
    ModrinthGameVersion[]
  >([]);
  ...
  const initialCategoriesState = useMemo(
    () =>
      ALL_MODRINTH_PROJECT_TYPES.reduce(
        (acc, pt) => ({ ...acc, [pt]: [] }),
        {} as Record<ModrinthProjectType, string[]>,
      ),
    [],
  );
  const [selectedCategoriesByProjectType, setSelectedCategoriesByProjectType] =
    useState(initialCategoriesState);

  const [selectedLoadersByProjectType, setSelectedLoadersByProjectType] =
    useState(initialCategoriesState);

  const [selectedGameVersions, setSelectedGameVersions] = useState<string[]>(
    [],
  );
  const [showAllGameVersionsSidebar, setShowAllGameVersionsSidebar] =
    useState(false); // Renamed state and set default to false
  ...
  const [expandedVersions, setExpandedVersions] = useState<
    Record<string, ModrinthVersion[] | null | "loading">
  >({});

  // New state for managing how many versions are displayed per project
  const [numDisplayedVersions, setNumDisplayedVersions] = useState<
  ...
  const [selectedVersion, setSelectedVersion] =
    useState<ModrinthVersion | null>(null);
  ...
  const [internalProfiles, setInternalProfiles] =
    useState<Profile[]>(initialProfiles);
  useEffect(() => {
    ...
    if (selectedProfileId && initialProfiles.length > 0) {
      const initiallySelectedProfile = initialProfiles.find(
  ...
  const availableLoaders = useMemo(() => {
    const loaders = allLoadersData.filter((loader) =>
  ...
  const dynamicFilterGroups = useMemo<UIDynamicFilterGroup[]>(() => {
    ...
    const categoriesForProjectType = allCategoriesData.filter(
      (cat) => cat.project_type === projectType,
    );
    const headers = [
      ...new Set(categoriesForProjectType.map((cat) => cat.header)),
    ];

    const groups = headers.map((header) => {
      const optionsForHeader = categoriesForProjectType.filter(
  ...
  const performSearch = useCallback(
    async (newSearch = false) => {
      ...
      console.log(
        "[ModrinthSearchV2] Proceeding with API call. Setting loading true.",
      );
      setLoading(true);
      setError(null);

      try {
        const response: ModrinthSearchResponse =
          await ModrinthService.searchProjects(
            searchTerm,
            projectType,
        ...
        setSearchResults((prevResults) =>
          newSearch ? response.hits : [...prevResults, ...response.hits],
        );
  ...
  useEffect(() => {
    console.log(
      "[ModrinthSearchV2] useEffect for search triggered. Calling performSearch(true). Params:",
      {
        searchTerm,
        projectType,
        categories: currentSelectedCategories,
  ...
  const handleCategoryToggle = (categoryName: string) => {
    const currentSelectionsForActiveType =
      selectedCategoriesByProjectType[projectType] || [];
    const wasPreviouslySelected =
      currentSelectionsForActiveType.includes(categoryName);
    ...
    setSelectedCategoriesByProjectType((prevGlobalSelections) => {
      ...
      const newGlobalSelections = {
        ...prevGlobalSelections,
        [projectType]: updatedSelectionsForCurrentType,
      };
      ...
      for (const otherPT of ALL_MODRINTH_PROJECT_TYPES) {
        ...
        if (wasPreviouslySelected) {
          ...
          if (selectionsForOtherPT.includes(categoryName)) {
            newGlobalSelections[otherPT] = selectionsForOtherPT.filter(
              (c) => c !== categoryName,
            );
          }
        } else {
          ...
          const categoryDefinitionForOtherPT = allCategoriesData.find(
            (catDef) =>
              catDef.name === categoryName && catDef.project_type === otherPT,
          );
          if (categoryDefinitionForOtherPT) {
            if (!selectionsForOtherPT.includes(categoryName)) {
              newGlobalSelections[otherPT] = [
                ...selectionsForOtherPT,
                categoryName,
              ];
            }
          }
  ...
  const handleLoaderToggle = (loaderName: string) => {
    const currentSelectionsForActiveType =
      selectedLoadersByProjectType[projectType] || [];
    const wasPreviouslySelected =
      currentSelectionsForActiveType.includes(loaderName);
    ...
    setSelectedLoadersByProjectType((prevGlobalSelections) => {
      ...
      for (const otherPT of ALL_MODRINTH_PROJECT_TYPES) {
        ...
        const selectionsForOtherPT = newGlobalSelections[otherPT] || [];
        const loaderDefinition = allLoadersData.find(
          (ldrDef) => ldrDef.name === loaderName,
        );
        ...
        if (wasPreviouslySelected) {
          ...
          if (
            loaderDefinition &&
            loaderDefinition.supported_project_types.includes(otherPT)
          ) {
            if (!selectionsForOtherPT.includes(loaderName)) {
              newGlobalSelections[otherPT] = [
                ...selectionsForOtherPT,
  ...
  const removeGameVersionTag = (version: string) =>
    handleGameVersionToggle(version);
  const removeLoaderTag = (loaderName: string) =>
    handleLoaderToggle(loaderName);
  const removeCategoryTag = (categoryName: string) =>
  ...
  const clearAllFilters = () => {
    ...
    setSelectedCategoriesByProjectType((prev) => ({
      ...prev,
      [projectType]: [],
    }));
  ...
  const checkDisplayedVersionsStatus = async (
    projectId: string,
  ...
  useEffect(() => {
    ...
    Object.entries(expandedVersions).forEach(([projectId, versions]) => {
      if (Array.isArray(versions) && versions.length > 0 && selectedProfile) {
        const displayCount =
          numDisplayedVersions[projectId] || initialDisplayCount;
  ...
  const installToProfile = async (profileId: string) => {
    ...
    try {
      const primaryFile =
        selectedVersion.files.find((file) => file.primary) ||
      ...
      const mappedContentType = mapModrinthProjectTypeToNrContentType(
        selectedProject.project_type as ModrinthProjectType,
      );
      ...
      toast.success(
        `Successfully installed ${selectedProject.title} (${selectedVersion.version_number}) to ${internalProfiles.find((p) => p.id === profileId)?.name || "profile"}`,
      );
      ...
      setInstalledProjects((prev) => ({
        ...prev,
        [selectedProject.project_id]: getStatusForNewInstall(
          prev[selectedProject.project_id],
        ),
      }));
    ...
    await toast
      .promise(
        async () => {
          ...
          if (!primaryFile) {
            throw new Error(
              "No download file available for the selected version.",
            );
          }

          const mappedContentType = mapModrinthProjectTypeToNrContentType(
            project.project_type as ModrinthProjectType,
          );
          if (!mappedContentType) {
            // The helper function shows its own toast for invalid types, so just throw to be caught by toast.promise.
            throw new Error(
              `Unsupported project type for installation: ${project.project_type}`,
            );
          }
          ...
          setInstalledProjects((prev) => ({
            ...prev,
            [project.project_id]: getStatusForNewInstall(
  ...
  const [quickInstallProject, setQuickInstallProject] =
    useState<ModrinthSearchHit | null>(null);
  const [quickInstallVersions, setQuickInstallVersions] = useState<
    ModrinthVersion[] | null
  >(null);
  const [quickInstallLoading, setQuickInstallLoading] = useState(false);
  const [quickInstallError, setQuickInstallError] = useState<string | null>(
    null,
  );

  // Helper function to map Modrinth project type to our ContentType enum
  function mapModrinthProjectTypeToNrContentType(
    ...
    switch (projectType) {
      ...
      case "modpack": // Modpacks are handled by creating a new profile
        toast.error(
          "Modpacks should be installed as new profiles, not as content via this method.",
        );
        return null;
      default:
        // Log unhandled project types if any, but avoid throwing error that breaks UI
        console.warn(
          `Unsupported Modrinth project type for direct installation: ${projectType}`,
        );
  ...
  const quickInstall = async (project: ModrinthSearchHit) => {
      ...
      try {
        await toast.promise(
          async () => {
            ...
            if (!bestVersion) {
              throw new Error(
                `No compatible version found for ${selectedProfile.name}`,
              );
            }

            // Find primary file for the best version
            const primaryFile =
              bestVersion.files.find((file) => file.primary) ||
              bestVersion.files[0];
            if (!primaryFile) {
          ...
          },
          {
            ...
            success: (data) =>
              `Successfully installed ${project.title} v${data.version} to ${selectedProfile.name}`,
            error: (err) =>
              `Failed to install: ${err instanceof Error ? err.message : String(err.message)}`,
          },
        );
    ...
    try {
      ...
      setQuickInstallError(
        `Failed to fetch versions: ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      setQuickInstallLoading(false);
    }
  ...
  const findBestVersionForProfile = (
    profile: Profile,
    versions: ModrinthVersion[],
  ): ModrinthVersion | null => {
  ...
  const quickInstallToProfile = async (profileId: string) => {
    ...
    try {
      const primaryFile =
        bestVersion.files.find((file) => file.primary) || bestVersion.files[0];
      if (!primaryFile) {
        toast.error("No download file available for the selected version.");
        setInstalling((prev) => ({ ...prev, [profileId]: false }));
        return;
      ...
      const mappedContentType = mapModrinthProjectTypeToNrContentType(
        quickInstallProject.project_type as ModrinthProjectType,
      );
      if (!mappedContentType) {
      ...
      toast.success(
        `Successfully installed ${quickInstallProject.title} (${bestVersion.version_number}) to ${profile.name}`,
      );
      ...
      setInstalledProjects((prev) => ({
        ...prev,
        [quickInstallProject.project_id]: getStatusForNewInstall(
          prev[quickInstallProject.project_id],
        ),
      }));
  ...
  const [openVersionDropdowns, setOpenVersionDropdowns] = useState<
    Record<string, { type: boolean; gameVersion: boolean; loader: boolean }>
  >({});
  ...
  const toggleVersionDropdown = (
    ...
    setOpenVersionDropdowns((prev) => {
      const currentProjectDropdowns = prev[projectId] || {
        type: false,
        gameVersion: false,
        loader: false,
      };
  ...
  const [versionDropdownUIState, setVersionDropdownUIState] = useState<
    Record<
      string,
      {
  ...
  const handleVersionDropdownUIChange = (
    projectId: string,
    field: keyof (typeof versionDropdownUIState)[string],
  ...
  const handleInstallModpackAsProfile = async (project: ModrinthSearchHit) => {
    if (project.project_type !== "modpack") {
      ...
      toast.error(
        "This handler is primarily for modpacks. For other types, behavior might differ.",
      );
      if (onInstallSuccess) {
        onInstallSuccess();
      }
    ...
    const toastId = toast.loading(
      `Fetching latest version for ${project.title}...`,
    );
    ...
    try {
      ...
      if (project.latest_version) {
        ...
        latestVersion =
          versions.find(
            (v) =>
              v.version_number === project.latest_version ||
              v.id === project.latest_version,
          ) || versions[0];
        if (!latestVersion && versions.length > 0) {
          latestVersion = versions.sort(
            (a, b) =>
              new Date(b.date_published).getTime() -
              new Date(a.date_published).getTime(),
          )[0];
      ...
      if (!latestVersion) {
        const allVersions = await ModrinthService.getModVersions(
          project.project_id,
        );
        if (allVersions && allVersions.length > 0) {
          latestVersion = allVersions.sort(
            (a, b) =>
              new Date(b.date_published).getTime() -
              new Date(a.date_published).getTime(),
          )[0];
      ...
      toast.loading(`Installing ${project.title} as new profile...`, {
        id: toastId,
      });
      ...
      toast.success(
        (t) => (
          <div className="flex flex-col">
            <span>
              Successfully installed {project.title} as a new profile!
            </span>
      ...
      try {
        ...
        console.error(
          "Failed to refresh profiles list internally:",
          profileError,
        );
      }
      ...
      toast.error(
        `Error installing ${project.title}: ${err.message || "Unknown error"}`,
        { id: toastId },
      );
    }
  };

  const handleInstallModpackVersionAsProfile = async (
    project: ModrinthSearchHit,
    version: ModrinthVersion,
  ) => {
    if (project.project_type !== "modpack") {
      ...
      toast.error(
        "This handler is primarily for modpack versions. For other types, behavior might differ.",
      );
      if (onInstallSuccess) {
        onInstallSuccess();
      }
    ...
    const toastId = toast.loading(
      `Installing ${project.title} (version ${version.version_number}) as new profile...`,
    );
  ...
  const handleInstallToNewProfile = async (
    ...
    const installationPromise = async () => {
      ...
      if (sourceProfileIdToCopy) {
        const sourceProfile = internalProfiles.find(
          (p) => p.id === sourceProfileIdToCopy,
        );
        const sourceProfileName = sourceProfile
          ? sourceProfile.name
          : "source profile";
      ...
      if (project.project_type === "modpack") {
        throw new Error(
          "Modpacks should be installed as new profiles, not as content to an existing one.",
        );
      }
    ...
    try {
      ...
      await toast.promise(
        installationPromise(),
        {
          ...
          success: (data) =>
            `${data.successMessageDetail} and installed ${data.projectTitle} v${data.versionNumber}!`,
  ...
  const handleDeleteVersionFromProfile = async (
    ...
    const profileName =
      internalProfiles.find((p) => p.id === profileId)?.name || profileId;

    const primaryFile =
      version.files.find((file) => file.primary) || version.files[0];
    if (!primaryFile) {
      toast.error(
        "No primary file found for the version. Cannot determine details for deletion.",
      );
      return;
    }
    ...
    if (!payload.sha1_hash) {
      toast.error(
        "SHA1 hash is missing for this version. Cannot proceed with deletion.",
      );
      console.error(
        "Deletion failed: SHA1 hash missing for",
        project.title,
      ...
      success: (data: any) => {
        ...
        setInstalledVersions((prev) => ({
          ...
          [version.id]: {
            is_installed: false,
            is_included_in_norisk_pack:
              prev[version.id]?.is_included_in_norisk_pack || false,
            is_specific_version_in_pack:
              prev[version.id]?.is_specific_version_in_pack || false,
        ...
        if (
          ...
          quickInstallProject?.project_id === project.project_id
        ) {
          setInstallStatus((prev) => ({ ...prev, [profileId]: false }));
        }
        ...
        const anyVersionsStillInstalled = Object.entries(
          installedVersions,
          ...
          const belongsToProject = versionProject.some(
            (v) => v.id === versionId,
          );
        ...
        if (!anyVersionsStillInstalled) {
          setInstalledProjects((prev) => ({
            ...
            [project.project_id]: {
              is_installed: false,
              is_included_in_norisk_pack:
                prev[project.project_id]?.is_included_in_norisk_pack || false,
              is_specific_version_in_pack:
                prev[project.project_id]?.is_specific_version_in_pack || false,
              is_enabled: null,
              found_item_details: null,
            },
          }));
    ...
    await toast
      .promise(
        ...
        },
        {
          ...
          success: ({ versionName }) =>
            `Successfully ${successMessage} ${project.title} (${versionName})`,
          error: (err) =>
            `Failed to ${toastMessage.toLowerCase()}: ${err.message || String(err)}`,
        },
      )
      .catch((err) => {
  ...
  return (
    ...
    <div
      className={`modrinth-search-v2 flex flex-row h-full gap-3 ${className}`}
    >
      {" "}
      {/* Added gap-3 */}
      ...
      <div className="left-content-area flex flex-col flex-1 overflow-hidden">
        ...
        <ModrinthSearchControlsV2
          ...
          sortOptions={sortOptions.map((opt) => ({
            // Map to SelectOption structure if not already
            value: opt.value,
            label: opt.label,
            icon: opt.icon,
          }))}
        ...
        <div className="search-results-area flex-1 overflow-y-auto p-4 space-y-3">
          ...
          {searchResults.map((hit) => {
            ...
            const currentOpenVersionDropdowns = openVersionDropdowns[
              hit.project_id
            ] || { type: false, gameVersion: false, loader: false };
            ...
            return (
              <ModrinthProjectCardV2
                ...
                isExpanded={
                  Array.isArray(projectVersions) && projectVersions.length > 0
                }
      ...
      {isSidebarVisible && (
        <ModrinthFilterSidebarV2
          ...
          onClientRequiredToggle={() =>
            setFilterClientRequired(!filterClientRequired)
          }
          filterServerRequired={filterServerRequired}
          onServerRequiredToggle={() =>
            setFilterServerRequired(!filterServerRequired)
          }
        />
      )}
