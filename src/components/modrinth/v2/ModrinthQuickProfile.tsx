...
export const ModrinthQuickProfile: React.FC<ModrinthQuickProfileProps> = ({
  ...
  return (
    <div className={cn("p-1 sm:p-2 md:p-4 space-y-4", isLoading && "opacity-70 pointer-events-none")}>
      <div className="text-center">
        <h3
          className="text-xl sm:text-2xl font-semibold text-gray-100 font-minecraft-ten normal-case"
        >
          {isActuallyCopying ? "Copy existing profile and install " : "Install "}
          <span style={{ color: accentColor.value }}>{projectTitle}</span>
          {versionNumber && <span className="text-gray-400"> v{versionNumber}</span>}
          <br />
      ...
      <div className="flex items-start gap-2">
        <div className="flex-grow">
          <label htmlFor="quickProfileNameInput" className="block text-sm font-medium text-gray-300 mb-1 sr-only">
            New Profile Name
          </label>
          <Input
      ...
      {error && <p id="profileNameError" className="text-red-500 text-xs mt-1 text-center">{error}</p>}

      {(showSourceSelectInput || isActuallyCopying) && (
          <div className="space-y-1 pt-2 animated-fade-in">
            <label htmlFor="sourceProfileSelect" className="block text-xs font-medium text-gray-400">
