...
import { CapeList, ADD_CAPE_PLACEHOLDER_ID } from './CapeList';
...
export function CapeBrowser() {
  ...
  const fetchCapesData = useCallback(async (pageToFetch: number, currentFilters: CapeFiltersData, currentSearchQuery: string, append: boolean = false) => {
    console.log('[CapeBrowser] fetchCapesData called with:', { pageToFetch, currentFilters, currentSearchQuery, append });
    if (append) {
    ...
    try {
      ...
      if (currentFilters.showOwnedOnly && activeAccount) {
        ...
        const playerCapesOptions: GetPlayerCapesPayloadOptions = { player_identifier: activeAccount.id };
        response = await getPlayerCapes(playerCapesOptions);
        setCapesData(response);
        setPaginationInfo({ currentPage: 0, pageSize: response.length, totalItems: response.length, totalPages: 1 });
      } else if (currentSearchQuery && currentSearchQuery.trim() !== '') {
        const playerCapesOptions: GetPlayerCapesPayloadOptions = { player_identifier: currentSearchQuery.trim() };
        response = await getPlayerCapes(playerCapesOptions);
        setCapesData(response);
        setPaginationInfo({ currentPage: 0, pageSize: response.length, totalItems: response.length, totalPages: 1 });
      } else {
        ...
        setCapesData(prevActualCapes => append ? [...prevActualCapes, ...response.capes] : response.capes);
  ...
  useEffect(() => {
    console.log('[CapeBrowser] Filter/Search useEffect. CurrentPage will be reset to 0 by handlers.');
    if (currentPage === 0) {
  ...
  useEffect(() => {
    if (currentPage > 0) {
        console.log(`[CapeBrowser] CurrentPage changed to ${currentPage}, fetching more items.`);
  ...
  const loadMoreCapes = useCallback(() => {
    if (hasMoreItems && !isFetchingMore) {
      console.log('[CapeBrowser] loadMoreCapes called. Current page:', currentPage, 'Pagination:', paginationInfo);
  ...
  const handleUploadClick = async () => {
    try {
      const selectedFile = await open({ multiple: false, directory: false, filters: [{ name: 'PNG Images', extensions: ['png'] }] });
  ...
  const handleDownloadTemplate = async () => {
    ...
    toast.promise(promise, {
      ...
      error: (err: any) => `Failed to download template: ${err.message || 'Unknown error'}`,
    });
  };
  ...
  return (
    ...
    >
        ...
        <Modal title="Preview & Upload Cape" onClose={handleCancelUpload} closeOnClickOutside={true} width="md">
          <div className="p-4">
            ...
            <div className="flex justify-center gap-4">
              ...
              <Button onClick={handleCancelUpload} variant="secondary" disabled={isUploading} size="lg">
                Cancel
              </Button>
            </div>
          </div>
      ...
      {showDeleteModal && capeToDelete && (
        <Modal title="Confirm Deletion" onClose={handleCancelDelete} width="sm">
          <div className="p-4">
            <p className="text-white/90 mb-6 text-center font-minecraft-ten">
              Are you sure you want to delete the cape <span className="text-[var(--accent)]">{capeToDelete._id}</span>?
              This action cannot be undone.
            </p>
            <div className="flex justify-center gap-4">
              <Button onClick={handleConfirmDelete} variant="destructive" disabled={isDeleting} size="md">
                {isDeleting ? 'Deleting...' : 'Delete Cape'}
              </Button>
              <Button onClick={handleCancelDelete} variant="secondary" disabled={isDeleting} size="md">
                Cancel
              </Button>
            </div>
          </div>
