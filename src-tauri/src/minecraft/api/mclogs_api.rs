...
pub async fn upload_log_to_mclogs(log_content: String) -> Result<MclogsUploadResult> {
    info!("Attempting to upload log to mclo.gs ({} bytes)", log_content.len());

    // Create the form data payload
    let params = [("content", log_content)];

    let response = HTTP_CLIENT.post(MCLOGS_API_URL)
        .form(&params)
        .send()
        .await
        .map_err(|e| {
            ...
            if data.success {
                ...
                match (&data.url, &data.id) {
                    ...
                    _ => {
                        ...
                        error!("mclo.gs API reported success but missing url or id. Response: {:?}", data);
                        Err(AppError::MclogsUploadFailed("API success but missing URL or ID".to_string()))
                    }
                }
            } else {
                 let error_msg = data.error.unwrap_or_else(|| "Unknown API error".to_string());
                error!("mclo.gs API reported failure: {}", error_msg);
                Err(AppError::MclogsUploadFailed(format!("API Error: {}", error_msg)))
            }
        }
        Err(e) => {
            error!("Failed to parse JSON response from mclo.gs: {}", e);
            Err(AppError::MclogsUploadFailed(format!("Failed to parse response: {}", e)))
        }
    }
} 