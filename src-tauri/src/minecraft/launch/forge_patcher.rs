...
impl ForgePatcher {
    ...
    fn replace_tokens(&self, arg: &str, install_profile: &ForgeInstallProfile, is_client: bool) -> Result<String> {
            ...
            match c {
                '\\' => {
                    ...
                    if let Some(next) = chars.next() {
                        ...
                        return Err(AppError::ForgeError("Illegal pattern (Bad escape)".to_string()));
                    }
                }
                '{' | '\'' => {
                    ...
                    if !found_end {
                        return Err(AppError::ForgeError(format!("Illegal pattern (Unclosed {}): {}", c, arg)));
                    }
                    ...
                    if c == '\'' {
                        ...
                        if let Some(value) = install_profile.data.get(&key) {
                            ...
                            match key.as_str() {
                                ...
                                _ => return Err(AppError::ForgeError(format!("Missing key: {}", key))),
                            }
                        }
                    }
                }
            ...
            if let Some(event_id) = self.event_id {
                if let Some(profile_id) = self.profile_id {
                    ...
                    state.emit_event(EventPayload {
                        ...
                        message: format!("Forge Processor: {} ({}/{})", processor.jar, i + 1, total_processors),
