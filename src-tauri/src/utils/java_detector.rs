...
use crate::utils::system_info::{OperatingSystem, OS, ARCHITECTURE, Architecture};
use regex::Regex;
...
lazy_static! {
    static ref JAVA_INSTALLATIONS: Arc<RwLock<Option<Vec<JavaInstallation>>>> = Arc::new(RwLock::new(None));
}
            ...
            match detect_java_on_windows().await {
                Ok(windows_javas) => {
                    for installation in windows_javas {
                        info!("Found Java on Windows: {} ({})", installation.path.display(), installation.version);
                        installations.push(installation);
                    }
                }
            ...
            match detect_java_on_macos().await {
                Ok(macos_javas) => {
                    for installation in macos_javas {
                        info!("Found Java on macOS: {} ({})", installation.path.display(), installation.version);
                        installations.push(installation);
                    }
                }
            ...
            match detect_java_on_linux().await {
                Ok(linux_javas) => {
                    for installation in linux_javas {
                        info!("Found Java on Linux: {} ({})", installation.path.display(), installation.version);
                        installations.push(installation);
                    }
                }
...
pub async fn get_java_info(java_path: &Path) -> Result<JavaInstallation> {
    ...
    if !java_path.exists() {
        return Err(AppError::Other(format!("Java path does not exist: {}", java_path.display())));
    }
    ...
    let vendor_regex = Regex::new(r#"(OpenJDK|Oracle|AdoptOpenJDK|Azul|Eclipse|GraalVM).*"#).unwrap();
    ...
    let version = version_regex.captures(&version_output)
        ...
        .ok_or_else(|| AppError::Other(format!("Failed to parse Java version from: {}", version_output)))?;
    
    // Parse the major version
    let major_version = parse_java_major_version(&version)
        .ok_or_else(|| AppError::Other(format!("Failed to parse Java major version from: {}", version)))?;
    
    // Determine vendor
    let vendor = vendor_regex.captures(&version_output)
        .and_then(|caps| caps.get(1))
        .map(|m| m.as_str().to_string())
        .unwrap_or_else(|| "Unknown".to_string());
    
    // Determine VM
    let vm_name = vm_regex.captures(&version_output)
        .and_then(|caps| caps.get(1))
        .map(|m| m.as_str().to_string());
...
pub async fn find_best_java_for_minecraft(mc_version: &str) -> Result<Option<JavaInstallation>> {
    ...
    let suitable = installations.iter()
    ...
    for path in path_var.split(path_separator) {
        ...
        if java_path.exists() {
            match get_java_info(&java_path).await {
                ...
                Err(e) => warn!("Failed to get info for Java in PATH {}: {}", java_path.display(), e),
            }
        }
    }
                    ...
                    if path.is_dir() {
                        ...
                        if java_exe.exists() {
                            match get_java_info(&java_exe).await {
                                ...
                                Err(e) => warn!("Failed to get info for Java at {}: {}", java_exe.display(), e),
                            }
                        }
                    }
                }
...
async fn detect_java_on_macos() -> Result<Vec<JavaInstallation>> {
    ...
    let locations = vec![
        ...
        // User Java
        PathBuf::from(format!("{}/Library/Java/JavaVirtualMachines", std::env::var("HOME").unwrap_or_default())),
    ];
