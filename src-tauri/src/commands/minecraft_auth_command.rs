...
pub async fn begin_login<R: Runtime>(
    ...
    let flow = State::get().await?.minecraft_account_manager_v2.login_begin().await?;
        ...
        if let Ok(url) = window.url() {
            if url.as_str().starts_with("https://login.live.com/oauth20_desktop.srf") {
                if let Some((_, code)) = url.query_pairs().find(|x| x.0 == "code") {
                    ...
                    let account = State::get().await?.minecraft_account_manager_v2.login_finish(&code, flow).await?;
...
pub async fn remove_account(account_id: Uuid) -> Result<(), CommandError> {
    ...
    state.minecraft_account_manager_v2.remove_account(account_id).await?;
    Ok(())
}
...
pub async fn get_active_account() -> Result<Option<Credentials>, CommandError> {
    let state = State::get().await?;
    let account = state.minecraft_account_manager_v2.get_active_account().await?;
    Ok(account)
}
...
pub async fn set_active_account(account_id: Uuid) -> Result<(), CommandError> {
    let state = State::get().await?;
    state.minecraft_account_manager_v2.set_active_account(account_id).await?;
    Ok(())
}
...
pub async fn get_accounts() -> Result<Vec<Credentials>, CommandError> {
    let state = State::get().await?;
    let accounts = state.minecraft_account_manager_v2.get_all_accounts().await?;
    Ok(accounts)
}