...
use tauri::{AppHandle, Manager, UserAttentionType, WebviewWindowBuilder, WebviewUrl, Url};
...
#[tauri::command]
pub async fn discord_auth_link(
    app: AppHandle,
) -> Result<(), CommandError> {
    ...
    let selected_account_arc = state
        ...
        .get_active_account()
        .await?
        .ok_or(AppError::AccountError("No active account found for Discord link.".to_string()))?;
    ...
    let external_url = Url::parse(&url_string)
        .map_err(|e| CommandError::from(AppError::Other(format!("Invalid URL format for Discord auth: {}", e))))?;

    if let Some(window) = app.get_webview_window("discord-signin") {
        debug!("Closing existing discord-signin window.");
        if let Err(e) = window.close().map_err(|e_close| CommandError::from(AppError::Other(format!("Failed to close existing Discord window: {}", e_close)))) {
    ...
    .build()
    .map_err(|e| CommandError::from(AppError::Other(format!("Failed to build Discord window: {}", e))))?;

    window.request_user_attention(Some(UserAttentionType::Critical))
        .map_err(|e| CommandError::from(AppError::Other(format!("Failed to request user attention for Discord window: {}", e))))?;
    debug!("Discord sign-in window opened.");

    while (Utc::now() - start_time) < ChronoDuration::minutes(10) {
        match window.url().map_err(|e| CommandError::from(AppError::Other(format!("Failed to get Discord window URL: {}",e )))) {
            Ok(current_url) => {
                let current_url_str = current_url.as_str();
                if current_url_str.starts_with("https://api.norisk.gg/api/v1/core/oauth/discord/complete") ||
                   current_url_str.starts_with("https://api-staging.norisk.gg/api/v1/core/oauth/discord/complete") {
                    debug!("Discord authentication successful, closing window.");
                    tokio::time::sleep(std::time::Duration::from_millis(1000)).await;
                    window.close().map_err(|e_close| CommandError::from(AppError::Other(format!("Failed to close Discord window after auth: {}", e_close))))?;
                    return Ok(());
                }
            }
            Err(e) => {
                debug!("Error getting window URL (assuming closed by user): {:?}", e);
                return Ok(());
            }
        }
    ...
    window.close().map_err(|e_close| CommandError::from(AppError::Other(format!("Failed to close Discord window after timeout: {}", e_close))))?;
    Ok(())
}
...
pub async fn discord_auth_status() -> Result<bool, CommandError> {
    ...
    let selected_account_arc = state
        ...
        .get_active_account()
        .await?
        .ok_or(AppError::AccountError("No active account found for Discord status check.".to_string()))?;
    
    let account_id_str = selected_account_arc.id.to_string();
    let norisk_creds = &selected_account_arc.norisk_credentials;
    let token = norisk_creds.get_token_for_mode(is_experimental)?;

    debug!("Checking Discord link status for account {} (experimental: {})", account_id_str, is_experimental);
...
pub async fn discord_auth_unlink() -> Result<(), CommandError> {
    ...
    let selected_account_arc = state
        ...
        .get_active_account()
        .await?
        .ok_or(AppError::AccountError("No active account found for Discord unlink.".to_string()))?;
